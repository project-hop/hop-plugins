...
public class VerticaBulkLoader extends BaseTransform<VerticaBulkLoaderMeta, VerticaBulkLoaderData> implements ITransform<VerticaBulkLoaderMeta, VerticaBulkLoaderData> {
    ...
    public VerticaBulkLoader(TransformMeta transformMeta, VerticaBulkLoaderMeta meta, VerticaBulkLoaderData data, int copyNr, PipelineMeta pipelineMeta,
                             Pipeline pipeline ) {
    ...
    private ColumnSpec getColumnSpecFromField( IValueMeta inputValueMeta, IValueMeta insertValueMeta,
    ...
    private String buildCopyStatementSqlString() {
        ...
        sb.append( databaseMeta.getQuotedSchemaTableCombination(variables, data.db.resolve( meta.getSchemaName() ),
        ...
        if ( !StringUtil.isEmpty( meta.getStreamName() ) ) {
            sb.append( "STREAM NAME E'" ).append( data.db.resolve( meta.getStreamName() ).replace( "'", "\\'" ) )
                    .append( "' " );
        }

        // XXX: I believe the right thing to do here is always use NO COMMIT since we want Hop's configuration to drive.
