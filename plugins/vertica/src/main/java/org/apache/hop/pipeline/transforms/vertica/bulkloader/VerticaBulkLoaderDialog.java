...
public class VerticaBulkLoaderDialog extends BaseTransformDialog implements ITransformDialog {
    ...
    public String open() {
        ...
        wFields =
                new TableView( variables, wFieldsComp, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI | SWT.V_SCROLL
        ...
        if ( missingSourceFields.length() > 0 || missingTargetFields.length() > 0 ) {
            ...
            int answer =
                    BaseDialog.openMessageBox(
                            shell,
                            BaseMessages.getString(PKG, "VerticaBulkLoaderDialog.DoMapping.SomeFieldsNotFoundTitle"),
                            message,
                ...
                if ( !StringUtil.isEmpty( wTable.getText() ) ) {
                    ...
                    if ( ci != null ) {
                        ...
                        try {
                            ...
                            String schemaTable =
                                    ci.getQuotedSchemaTableCombination( variables, variables.resolve( wSchema.getText() ), variables.resolve( wTable.getText() ) );
...
//        int connr = wConnection.getSelectionIndex();
        ...
        if(databaseMeta != null){
            log.logDebug( toString(), BaseMessages.getString( PKG, "VerticaBulkLoaderDialog.Log.LookingAtConnection", databaseMeta
                    .toString() ) );
    ...
    private void sql() {
        try {
            ...
            if ( !sql.hasError() ) {
                if ( sql.hasSql() ) {
                    SqlEditor sqledit =
                            new SqlEditor( shell, SWT.NONE, variables, info.getDatabaseMeta(), DbCache.getInstance(), sql.getSql() );
                    sqledit.open();
                } else {
